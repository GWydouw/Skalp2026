  ...
  module Skalp
    def self.remove_wrong_rgloader
      $LOADED_FEATURES.select{ |i| i.include?('rgloader')}.each do |path|
        unless path.include?('Skalp_Skalp')
    ...
    SKALP_PATH = Sketchup.find_support_file("Plugins")+"/Skalp_Skalp/"
    SKALP_VERSION = '#SKALPVERSION#'
    ...
    Sketchup::require 'Skalp_Skalp/Skalp_translator'

    options={
        :custom_path => Sketchup.find_support_file("Plugins")+"/Skalp_Skalp/resources/strings/",
        :debug => false
    }
    ...
    def self.load_extension
      if @ie_check == false
        ...
        @skalp_extension = SketchupExtension.new "Skalp", "Skalp_Skalp/Skalp_loader.rb"
        @skalp_extension.version = SKALP_VERSION

        @skalp_extension.copyright = "2014-2025, Skalp"
