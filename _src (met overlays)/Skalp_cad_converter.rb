module Skalp
  module Cad_File_Converter
    ...
    class << self
      attr_accessor :teigha_path, :input_path, :output_path, :output_version, :output_file_type, :recurse_input_older, :audit_each_file
      attr_accessor :input_file_filter #[optional] Input file filter (default:"*.DWG;*.DXF")
    end
    ...
    def self.install_teigha
      ...
      if Sketchup.is_online
        ...
        begin
          ...
          UI.messagebox("Error installing DXF/DWG converter: #{error}")
          return false
        end
    ...
    def self.check_teigha
      if teigha_path && File.file?(teigha_path)
        return
      else
        if Sketchup.platform == :platform_osx
    ...
    def self.teigha_cmd
      if Sketchup.platform == :platform_win
        ...
        cmd = %Q("#{path}" #{input_path_win} #{output_path_win} "#{output_version}" "#{output_file_type}" "#{recurse_input_older}" "#{audit_each_file}")
        cmd.encode('utf-8')
