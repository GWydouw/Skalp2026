module Skalp
  ...
  def self.buy_maintenance(guid)
    ...
    case seats <=> 1
      ...
      when -1
        UI.messagebox("#{Skalp.translate('Zero seats available on your license. Please contact Skalp support.')}", MB_OK)

      # single seat license
      when 0
        UI.openURL("https://sites.fastspring.com/skalp/product/buy_maintenance?referrer=#{guid}")

      # multi seat license
      when 1
        UI.openURL("https://sites.fastspring.com/skalp/product/buy_maintenance_plus?referrer=#{guid}")
      else
        # error handling
    end
  rescue
  ...
  def self.new_version()
    ...
    if Sketchup.is_online
      ...
      if version.to_i == 1
        ...
        if (your_version < newest_version)
          update_needed = true
        end
  ...
  def self.deactivate
    unless File.exist?(Sketchup.find_support_file("Plugins")+"/Skalp_Skalp/Skalp.lic")
  ...
  def self.uninstall(update=false)
    ...
    if result == 6
      ...
      if @status == 1
        ...
        @skalp_toolbar.hide if @skalp_toolbar
      end
      uninstall_status = 1

      #delete subdirs except resources
      FileUtils.remove_dir(PLUGIN_PATH + '/Skalp_Skalp/html', true)
      FileUtils.remove_dir(PLUGIN_PATH + '/Skalp_Skalp/chunky_png', true)

      #delete resources/strings
      FileUtils.remove_dir(PLUGIN_PATH + '/Skalp_Skalp/resources/strings', true)

      #delete files
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/LICENSE.txt', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_geom.rbs', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_geom.rb', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_log.txt', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_lib.rbs', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_geom2.rb', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_lib2.rb', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_loader.rb', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_observers.rb', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_preferences.rb', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_update.rb', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_translator.rb', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_version.rb', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_UI.rb', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_isolate.rbs', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_info.rb', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_lib.rb', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_lic.rb', true)
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/Skalp_Skalp.hash', true)

      #delete Skalp.rb
      FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp.rb', true)

      uninstall_status = 2

      #delete C-extensions
      # FileUtils.remove_dir(PLUGIN_PATH + '/Skalp_Skalp/rgloader_old', true)
      # FileUtils.mv(PLUGIN_PATH + '/Skalp_Skalp/rgloader', PLUGIN_PATH + '/Skalp_Skalp/rgloader_old')
      #
      # FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/SkalpC_old.bundle',true) if File.exist?(PLUGIN_PATH + '/Skalp_Skalp/SkalpC_old.bundle')
      # FileUtils.remove_entry(PLUGIN_PATH + '/Skalp_Skalp/SkalpC_old.so',true) if File.exist?(PLUGIN_PATH + '/Skalp_Skalp/SkalpC_old.so')
      #
      # File.rename(PLUGIN_PATH + '/Skalp_Skalp/SkalpC.bundle', PLUGIN_PATH + '/Skalp_Skalp/SkalpC_old.bundle' )
      # File.rename(PLUGIN_PATH + '/Skalp_Skalp/SkalpC.so', PLUGIN_PATH + '/Skalp_Skalp/SkalpC_old.so' )
      return true
    else
      return false
    end
  rescue
    UI.messagebox("#{Skalp.translate('Sorry, automatic uninstall failed.')} #{Skalp.translate('Please uninstall Skalp manually.')}", MB_OK) if uninstall_status < 2
    return false
  end
