...
# implements methods to create previews and tileable png textures.
    class Hatch
      ...
      def add_hatchdefinition(hatchdef)
        ...
        @def_x, @def_y, @def_offset = def_dims[0], def_dims[1], def_dims[2]
      end
      ...
      def create_png(opts={})
        ...
        if @opts[:type] == :preview  || @opts[:type] == :thumbnail#PREPARE FOR PREVIEW PNG

          #puts "PREVIEW_________________________#{hatchdefinition.name}"
          height * def_x / def_y < width ?
              scale_factor = height / def_y :
              scale_factor = width / def_x
        ...
        if SkalpHatch.develop
          ...
          if hatchdefinition.def_normalisation > 1
            #return
            scale *= hatchdefinition.def_normalisation #FIXME TEMPORARY HACK, PRODUCES NON TILING RESULT! #FOGBUGZ CASE 433
          else
            #return
          end
        end
        ...
        pen_width = pen_width * resolution
        ...
        draw_section_cut(png, section_cut_width) if @opts[:type] == :preview ||  @opts[:type] == :thumbnail
        save_png(png, hatchdefinition.name) if @opts[:type] == :preview || @opts[:type] == :tile
        ...
        return {:gauge_ratio => @def_y / @def_x, :original_definition => @hatchdefinition.originaldefinition, :pat_scale => user_x / def_x_or_y} if @opts[:type] == :tile
      end
      ...
      def parse_colors
        if @opts[:line_color].include?('rgba')
          rgba = @opts[:line_color]
          rgba_factor = rgba.gsub("rgba","").gsub("(","").gsub(")","").split(',')
          @opts[:line_color] = "rgb(" + rgba_factor[0] + "," + rgba_factor[1] + "," + rgba_factor[2] + ")"
        end

        @opts[:line_color] = eval("ChunkyPNG::Color::" + @opts[:line_color]) if @opts[:line_color] # solid_color has no line_color

        fill_color_values = @opts[:fill_color].scan(%r((\d+(?:\.\d+)?|\.\d+))).flatten   #.scan(%r(\d+(?:.\d+)?)) zie mail Guy dd 2017.02.25 14:53 en http://stackoverflow.com/questions/11500482/regex-to-find-integers-and-decimals-in-string
        if fill_color_values.size == 4
          fill_color_values[3] = (fill_color_values[3].to_f * 255).to_i.to_s
          @opts[:fill_color] = "rgba(#{fill_color_values.join(", ")})"
        end
        @opts[:fill_color] = eval("ChunkyPNG::Color::" + @opts[:fill_color])
      end
      ...
      def save_png(png, name)
        ...
        return png.save(File.expand_path('~') + "/Desktop/hatchtextures/#{@opts[:type].to_s}_#{name}.png", png_constraints) if SkalpHatch.develop # external run
        png.save(Skalp::IMAGE_PATH + "#{@opts[:type].to_s}.png", png_constraints)
      end
      ...
      def repeat_points(point, pointline, pen_width, png, hl, scale)
        ...
        t1 = Transformation2D.new.translation(hl.line_style_length_vector.x * scale, hl.line_style_length_vector.y * scale)
        t2 = t1.dup.inverse
        ...
        if inside_hatchtile_edge.class == Point2D # a Hatchtile cornerpoint
          ...
          until started
            ...
            if (param1 < 0 && param2 > 1)||(param2 < 0 && param1 > 1)
              ...
              if inside_hatchtile_edge.point_on_edge?(newdash.p1) || inside_hatchtile_edge.point_on_edge?(newdash.p2)
                started = true
                draw_an_edge_or_line(newdash, pen_width, png)
                ...
                return if inside_hatchtile_edge.position_on_edge?(newdash.p1) > 1 && inside_hatchtile_edge.position_on_edge?(newdash.p2) > 1

              elsif param1 > 1 && param2 > 1
                newdash.transform!(t2)
                return if inside_hatchtile_edge.position_on_edge?(newdash.p1) < 0 && inside_hatchtile_edge.position_on_edge?(newdash.p2) < 0
              end
            end
          end
        end
      ...
      def draw_an_edge_or_line(object, pen, png, stroke_color = @opts[:line_color])
        object = object.dup.transform!(@comp_trans) unless @opts[:type] == :preview ||  @opts[:type] == :thumbnail
        edge = object.dup
