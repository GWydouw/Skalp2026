module Skalp
  ...
  def sort_section_table(table)
    sorted_table = table.sort do |a, b|
  ...
  def to_boolean(string)
    return string if string.class == TrueClass || string.class == FalseClass
    string == 'true'
  end
  ...
  def change_active_sectionplane(sectionplane)
    ...
    if sectionplane == "- #{NO_ACTIVE_SECTION_PLANE} -" || skalp_sectionplane == nil
      ...
      if skpSectionplane.valid? && skpSectionplane.parent.valid? && Skalp.active_model.skpModel.active_entities == skpSectionplane.parent.entities
        ...
        Skalp.active_model.set_memory_attribute(Sketchup.active_model, 'Skalp', 'active_sectionplane_ID', skpSectionplane.get_attribute('Skalp', 'ID'))
      end
    end
  end
  ...
  def layer_check(layername)
    ...
    layername.upcase[0..24].gsub(' ', '_').gsub(/[.,&<>"':;\|=\/\\\*]/, '_')
  ...
  def get_skalp_pattern_layer_definition
    ...
    definitions.each do |definition|
      if definition.get_attribute('Skalp', 'layers') == '1'
  ...
  def update_page(page)
    ...
    mask = 115
    mask = mask + 4 if page.use_shadow_info?
    mask = mask + 8 if page.use_axes?
  ...
  def unique_style_name(style_name)
    ...
    return basename + num_match.to_s
  end
  ...
  def force_style_to_show_skalp_section(skpPage)
    if skpPage.rendering_options['SectionCutFilled']
      styles = Sketchup.active_model.styles
      ...
      mask = 115
      mask = mask + 4 if skpPage.use_shadow_info?
      mask = mask + 8 if skpPage.use_axes?
  ...
  def make_new_style_from_active_settings
    ...
    result = UI.inputbox(["Style changed"], ["Update the selected style"], ["Update the selected style|Save as a new style"],  "Warning - Scenes and Styles")

    if result
      if result[0] == 'Save as a new style'
        default = Sketchup.find_support_file("Plugins")+"/Skalp_Skalp/resources/SUStyles/default.style"
  ...
  def equal_cameras?(cam1, cam2)
    ...
    if cam1.perspective? == cam2.perspective?
      if cam1.perspective? && cam2.perspective?
        return false if cam1.focal_length != cam2.focal_length || cam1.fov != cam2.fov
      end
    else
      return false
    end
  ...
  def check_skalp_pattern_style(object)
    case object
      when Sketchup::Model
        ...
        rules = eval(attrib)
      when Sketchup::Page
        if Skalp.active_model.get_memory_attribute(object, 'Skalp', 'style_layer') != '' && Skalp.active_model.get_memory_attribute(object, 'Skalp', 'style_layer') != nil
          rules = eval(Skalp.active_model.get_memory_attribute(object, 'Skalp', 'style_layer'))
        else
          return false if Skalp.active_model.get_memory_attribute(Sketchup.active_model, 'Skalp', 'style_layer') == '' || Skalp.active_model.get_memory_attribute(@skpModel, 'Skalp', 'style_layer') == nil
          rules = eval(Skalp.active_model.get_memory_attribute(Sketchup.active_model, 'Skalp', 'style_layer'))
        end
    end

    rules.each do |rule_string|
      if rule_string.class == Hash
        rule = rule_string
      else
        rule = eval(rule_string)
      end

      if rule[:layer] == "Skalp Pattern Layer"
        return true
      end
    end
  ...
  def create_Color_by_Layer_layers(materials = Sketchup.active_model.materials, update = false)
    ...
    if update
      layer_names.each do |layername|
        if layers[layername] && layers[layername].valid?
          to_delete << layers[layername]
        end
      end
    else
  ...
  def remove_scene_name(filename)
      Sketchup.active_model.pages.each do |page|
        next unless page
        if filename.include?("-#{page.name}")
          return filename.gsub("-#{page.name}", '')
        end
      end
    return filename
  end
  ...
  def create_layer_materials(temp_dir, layer_names)
    require "Skalp_Skalp/shellwords/shellwords"
    ...
    if OS == :WINDOWS
      command = %Q("#{path[1..-2]}Skalp.exe" "create_layer_materials" "#{temp_dir}" "#{layer_names_base64}")
    else
      command = %Q(#{path}Skalp "create_layer_materials" "#{temp_dir}" "#{layer_names_base64}")
    end
  ...
  def modifyStyle(temp_path, new_path)
    ...
    if OS == :WINDOWS
      command = %Q("#{path[1..-2]}Skalp.exe" "modifyStyle" "#{temp_path}" "#{new_path}" )
    else
      command = %Q(#{path}Skalp "modifyStyle" "#{temp_path}" "#{new_path}")
    end
  ...
  def setup_reversed_scene(temp_dir, new_temp_dir, index_array, reversed_eye_array, reversed_target_array, transformation_array, group_id_array, up_vector_array, modelbounds)
    require "Skalp_Skalp/shellwords/shellwords"

    path = Shellwords::escape(SKALP_PATH + "lib/")

    if OS == :WINDOWS
      command = %Q("#{path[1..-2]}Skalp.exe" "setup_reversed_scene" "#{temp_dir}" "#{new_temp_dir}" "#{array_to_string_array(index_array)}" "#{point_array_to_string_array(reversed_eye_array)}" "#{point_array_to_string_array(reversed_target_array)}" "#{point_array_to_string_array(transformation_array)}" "#{array_to_string_array(group_id_array)}" "#{point_array_to_string_array(up_vector_array)}" "#{modelbounds.to_s}")
    else
      command = %Q(#{path}Skalp "setup_reversed_scene" "#{temp_dir}" "#{new_temp_dir}" "#{array_to_string_array(index_array)}" "#{point_array_to_string_array(reversed_eye_array)}" "#{point_array_to_string_array(reversed_target_array)}" "#{point_array_to_string_array(transformation_array)}" "#{array_to_string_array(group_id_array)}" "#{point_array_to_string_array(up_vector_array)}" "#{modelbounds.to_s}")
    end
  ...
  def get_exploded_entities(temp_dir, height, index_array, scale_array, perspective_array, target_array, rear_view)
    ...
    if OS == :WINDOWS
      command = %Q("#{path[1..-2]}Skalp.exe" "get_exploded_entities" "#{temp_dir}" "#{height}" "#{array_to_string_array(index_array)}" "#{array_to_string_array(scale_array)}" "#{array_to_string_array(perspective_array)}" "#{point_array_to_string_array(target_array)}" "#{rear_view.to_s}") #--silent
    else
  ...
  def process_active?
    ...
    for proces in processes
      if proces.Name == 'Skalp.exe'
        pid = proces.ProcessId
      end
    end
  ...
  def inch_to_modelunits(value)
    ...
    case Sketchup.active_model.options["UnitsOptions"]["LengthFormat"]
      when 0
        factor = Sketchup::format_length(1000000).gsub('~', '').to_f/1000000 #decimal
      when 1
        factor = 1.0 #architectural
      when 2
        factor = 1.0 #engineering
      when 3
        factor = Sketchup::format_length(1000000).gsub('~', '').to_f/1000000 #fractional
    end
  ...
  def aligned(e)
    ...
    if info then
      (info[:alignment] == 'true') ? true : false
    else
      false
    end
  end
  ...
  def key(flags, key=0, status=:no_status)
    ...
    if OS == :MAC
      ...
      case
        ...
        when alt #+
          return :alt
        when command # +- in component
          return :command
        else
          return :no_key
      end
    else
      @shift = false unless @shift
      @control = false unless @control
      @alt = false unless @alt
      ...
      case
        when @shift && @control && !@alt # -
          return :shift_alt
        when @shift && !@control && !@alt #+-
          return :shift
        when !@shift && @control && !@alt #+
          return :alt
        when !@shift && !@control && @alt # +- in component
          return :command
        else
          return :no_key
      end
  ...
  def pen2inch(pen)
    if pen.class == Float
     return pen
      ...
      return pen.gsub('pt','').gsub(' ','').to_f / 72.0
    elsif pen.include?('mm')
      return pen.gsub('mm','').gsub(' ','').to_f / 25.4
    else
      return pen.to_f
    end
  end
  ...
  def mm_or_pts_to_inch(pen)
    ...
    if pen.include?('mm') #pen_paper
      ...
      pen_width = 0.070866 unless pen_width
    else
  ...
  def set_ID(entity)
    ...
    if entity.class == Sketchup::Page
      ...
      unless entity.get_attribute('Skalp', 'ID')
        entity.set_attribute('Skalp', 'ID', generate_ID)
      end
    end

    return Skalp.get_ID(entity)
  end
  ...
  def get_definition_entities(object)
    ...
    return object.entities if object.is_a?(Sketchup::Group)
  ...
  def material_inside_object(object)
    return object.material if object.material != nil

    entities = get_definition_entities(object)
    materials = Set.new

    entities.grep(Sketchup::Face).each do |face|
      face.material ? materials << face.material : materials << face.back_material
    end

    if materials.size == 1
      if materials.first && materials.first.valid?
        return materials.first
      else
        return nil
      end
    else
      if object.material && object.material.valid?
  ...
  def object?(entity)
    return false if not entity.valid?
    ...
    if entity.is_a?(Sketchup::Group) || entity.is_a?(Sketchup::ComponentInstance)
      for parentinstance in entity.parent.instances
        if parentinstance.parent == Sketchup.active_model
          ...
          self.find_toplevel_parents(parentinstance, parents_array) if parentinstance.parent != Sketchup.active_model
        end
      end
    end
  ...
  def self.get_rearview_linestyle_by_tag
    linestyle = {}
    Sketchup.active_model.layers.each do |layer|
      if layer.name.include?('Skalp rearview - ')
  ...
  def create_skalp_default_material(name = "Skalp default", line_color = "rgb(110, 110, 110)", fill_color = "rgb(244, 244, 244)")
    Skalp.active_model ? active_model.start("Skalp - #{Skalp.translate('Create default material')}", true) : Sketchup.active_model.start_operation("Skalp - #{Skalp.translate('Create default material')}", true, false, false)
  ...
  def check_skalp_default_material
    ...
    materials = Sketchup.active_model.materials
    unless materials["Skalp transparent"]
  ...
  def rendering_options?(object)
    ...
    if object.use_rendering_options?
      return object
    else
  ...
  def edit_skalp_material(hatchname)
    if Sketchup.active_model == Skalp.active_model.skpModel && hatchname != ''
      material = Sketchup.active_model.materials[hatchname]

      if material && material.get_attribute('Skalp', 'ID')
