...
=end
  ...
  def update_skalp_materials_definition(componentdefinition)
    ...
    Skalp.active_model.used_materials.each do |material|
      if materials_in_model.include?(material)
        ...
        missing_materials << material if material != nil
      end
    end
  ...
  def add_skalp_material_to_instance(materialnames)
    materialnames.each do |materialname|
      ...
      unless Skalp.active_model.used_materials.include?(material)
        Skalp.active_model.used_materials << material
      end
    end
  ...
  def find_skalp_materials_definition
    Sketchup.active_model.definitions.each do |definition|
      if definition.name == 'skalp_materials'
        return definition
      end
    end
    return nil
  ...
  def create_su_material(material_name)
    ...
    material_name = 'Skalp default' if material_name == '' || material_name == nil
    ...
    if su_material == nil || !su_material.get_attribute('Skalp', 'ID')
      if color_from_name(material_name)
        Sketchup.active_model.set_attribute('Skalp_sectionmaterials', material_name, {fill_color: "#{color_from_name(material_name)}"}.inspect)
  ...
  def Skalp.convert_old_libraries_to_json
    ...
    cache_files.each do |cache_file|
      ...
      lines.each do |line|
        begin
          ...
          if info[:line_color] == info[:fill_color]
            pattern[0] = "*SOLID_COLOR, solid color without hatching"
          end
  ...
  def remove_scaled_textures
    model = Sketchup.active_model
    return unless Skalp.active_model.get_memory_attribute(model, 'Skalp', 'scaled_materials').to_i == 1
  ...
  def export_material_textures(layout = false)
    ...
    if layout
      path = SKALP_PATH + "resources/Layout Pattern-Fill Images/Skalp Patterns/"
    else
      path = SKALP_PATH + "resources/material_images/"
    end
  ...
  def check_syntax_skalp_material(material)
    ...
    (name == skalp_name) ? set_pattern_info_attribute(material, get_pattern_info(material)) : set_skalp_material_info(material, :name, name)
  end

  def version_compare
    @version_compare_flag ||= SKALP_VERSION == Skalp.active_model.get_memory_attribute(Sketchup.active_model, 'Skalp', 'skalp_version')
  end
  ...
  def check_SU_material_library(material = nil)
    ...
    material ? check_syntax_skalp_material(material) : Sketchup.active_model.materials.each { |material| check_syntax_skalp_material(material) }
  ...
  def set_skalp_material_info(su_material, info_type, new_setting)
    ...
    if material_ID
      ...
      if pattern_info.class == Hash
        pattern_string = pattern_info
      else
        pattern_string = eval(pattern_info)
      end
      updated_string = {}

      pattern_string.each_pair do |key, value|
        if key == info_type
          updated_string[key] = new_setting
        else
          updated_string[key] = value
        end
      end
  ...
  def material_brightness(material)
    ...
    if material.get_attribute('Skalp', 'ID')
      ...
      fill > line ? brightness = fill : brightness = line
    else
...
<Parameter Name=\"Emitter/Passive\" Type=\"Boolean\" Value=\"1\"/>\n")
  ...
  def create_thumbnails_cache(thumbnails = false)
    ...
    materials.each do |material|
      ...
      pattern_info = eval(material.get_attribute('Skalp', 'pattern_info')) rescue nil
      ...
      unless pattern_info[:png_blob]
        ...
        if png_blob
          ...
          material.set_attribute('Skalp', 'pattern_info', "eval(Sketchup.active_model.get_attribute('Skalp', 'version_check').to_s);#{pattern_info.inspect}")
        end
      end
...
def copy_material_between_libraries(source_library, target_library, materialname)
  ...
  source_data = JSON.parse(File.read(source_path)) rescue []
  target_data = JSON.parse(File.read(target_path)) rescue []
