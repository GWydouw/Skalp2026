module Skalp
  ...
  def import_skalp_materials
    ...
    materials.each do |material|
      if material.get_attribute('Skalp', 'ID')
        if material.name[-1] == '1' && ori_materials_names.include?(material.name[0..-2])
          to_delete << material
        end
      end
    end
  ...
  def choose_library
    ...
    if result[0] == "*** create new library ***"
      ...
      return MATERIAL_PATH + result[0] + '.skp'
    else
      false
    end
  ...
  def import_layer_mapping
    ...
    mapping_attribute = mapping_definition.entities.grep(Sketchup::ConstructionPoint)[0].get_attribute('Skalp', 'layermapping')
  ...
  def save_pattern_to_library(materialname)
    ...
    if Skalp::Material_dialog.active_library == 'Skalp materials in model'
      if Sketchup.active_model.materials[materialname] &&
        ...
        pattern_info = eval(Sketchup.active_model.materials[materialname].get_attribute('Skalp', 'pattern_info')) rescue nil
      end
    else
  ...
  def export_materials(materials, filename = nil, mapping = nil)
    ...
    materials.each do |material|
      ...
      face = definition.entities.add_face([0 + n, 0 + m, 0], [50 + n, 0 + m , 0], [50 + n, 50+m, 0], [0 + n, 50 + m, 0])
  ...
  def delete_skalp_material(name, library_name = nil)
    ...
    if library_name == 'Skalp materials in model'
      ...
      Skalp::Material_dialog.create_thumbnails if Skalp::Material_dialog.respond_to?(:create_thumbnails)
      return
    end
  ...
  def save_all_skalp_materials_to_new_library
    ...
    materials.each do |mat|
      pattern_info = eval(mat.get_attribute('Skalp', 'pattern_info')) rescue nil
      next unless pattern_info.is_a?(Hash)

      unless pattern_info[:png_blob]
        png_blob = Skalp.create_thumbnail(pattern_info) rescue nil
  ...
  def rename_material_in_library(library_name, old_name)
    return if library_name == 'Skalp materials in model' || library_name == 'SketchUp materials in model'
