module Skalp
  class SkalpEntitiesObserver < Sketchup::EntitiesObserver
    @@watched_types = Array[Sketchup::ComponentInstance, Sketchup::Group, Sketchup::Face, Sketchup::SectionPlane]
    ...
    def onElementModified(entities, entity)
      ...
      if entity.is_a?(Sketchup::SectionPlane)
        ...
        if Skalp::new_sectionplane == entity && (id == nil || id == '') && (entity.name && entity.name!='') && (entity.symbol && entity.symbol!='')
  ...
  class SkalpPagesObserver < Sketchup::PagesObserver
    ...
    def onElementAdded(pages, page)
      ...
      return if Skalp.status == 0
      return if not Skalp.models[pages.model].observer_active
  ...
  class SkalpViewObserver < Sketchup::ViewObserver
    ...
    def onViewChanged(view)
      ...
      if @model.entities.active_section_plane
        if parallel_state_changed?(view)
          ...
          if Skalp.dialog.fog_status(current_page)
            ...
            @model.rendering_options['DisplayFog'] = false  if @parallel_current_state == :non_parallel
          end
        end
      end
    ...
    def parallel_state_changed?(view)
      ...
      (@parallel_previous_state == @parallel_current_state) ? false : true
    end
  ...
  class SkalpLicenseObserver < Sketchup::AppObserver
    def onQuit
      if Sketchup.read_default('Skalp', 'guid') && File.exist?(Sketchup.find_support_file("Plugins")+"/Skalp_Skalp2026/Skalp.lic") && Skalp.check_license_type_on_server(Sketchup.read_default('Skalp', 'guid')) == 'network'
  ...
  class SkalpAppObserver < Sketchup::AppObserver
    ...
    def onActivateModel(model)
      ...
      return unless model.valid?
      return unless model
      if Skalp.status == 1 && Skalp.model_collection.include?(model)
        Skalp.change_active_model(model)
      end
    end
  end
  ...
  class SkalpSelectionObserver < Sketchup::SelectionObserver
    ...
    def onSelectionCleared(selection)
      return if Skalp.status == 0
      return if not Skalp.models[selection.model].observer_active
  ...
  class SkalpLayersObserver < Sketchup::LayersObserver
    def onLayerAdded(layers, layer)
      ...
      return if Skalp.status == 0
      return if not Skalp.models[layers.model].observer_active
  ...
  class SkalpMaterialsObserver < Sketchup::MaterialsObserver
    def onMaterialChange(materials, material)
      ...
      return if not Skalp.models[materials.model].observer_active
      return if not Skalp.models[materials.model].material_observer_active
