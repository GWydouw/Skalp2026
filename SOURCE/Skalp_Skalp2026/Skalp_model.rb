module Skalp
  class Model
    ...
    def initialize(skpModel)
      ...
      if get_memory_attribute(@skpModel, "Skalp",
                              "active_sectionplane_ID") && get_memory_attribute(@skpModel, "Skalp", "active_sectionplane_ID").slice(
                                0, 1
                              )
    ...
    def load_multitags_materials
      ...
      unless File.exist?(json_file)
        ...
        data << %w[status classificatie materiaal]
    ...
    def setup_skalp_folders
      ...
      @skalp_folder.folders.each do |folder|
        ...
        next unless folder.valid? && folder.get_attribute("Skalp", "folder") == "Rearview"
    ...
    def create_status_on_undo_stack
      ...
      if @skpModel.entities.active_section_plane
        set_memory_attribute(@skpModel, "Skalp", "active_sectionplane_ID",
                             @skpModel.entities.active_section_plane.get_attribute("Skalp", "ID"))
      else
        set_memory_attribute(@skpModel, "Skalp", "active_sectionplane_ID", "")
      end
    ...
    STYLE_SETTINGS_KEYS = %i[drawing_scale rearview_status rearview_linestyle section_cut_width_status
          ...
          is_meaningful = object.is_a?(Sketchup::Model) ||
                          reconstructed.any? do |k, v|
                            ...
                            when :rearview_status, :section_cut_width_status, :depth_clipping_status
                              v == true
                            when :style_rules
                              if v.is_a?(Skalp::StyleRules)
                                !v.rules.empty?
                              else
                                (v.is_a?(Array) ? !v.empty? : false)
                              end
    ...
    def delete_memory_attribute(object, dict_name)
      ...
      @memory_attributes[object] = {}
      return unless object.attribute_dictionaries && object.attribute_dictionaries[dict_name]
    ...
    def load_prefs
      @skalp_version = get_memory_attribute(@skpModel, "Skalp",
    ...
    def check_sectionplane_name(name)
      ...
      check = true
      @sectionplanes.each_value do |value|
        next unless value
    ...
    def sectionplane_by_id(skalpID)
      ...
      @sectionplanes.each_value do |sectionplane|
        return sectionplane if sectionplane.skalpID == skalpID && !sectionplane.skalpID.nil?
      end
      nil
    ...
    def set_sectionplane_layers_off
      ...
      if Skalp.sectionplane_active == false
        @section_result_group.entities.grep(Sketchup::Group).each do |section_group|
          ...
          section_group.hidden = true if section_group.get_attribute("Skalp", "ID") == "skalp_live_sectiongroup"
        end
      end
    ...
    def unload_observers
      ...
      if @skpModel.rendering_options && @render_options_observer
        @skpModel.rendering_options.remove_observer(@render_options_observer)
      end
    ...
    def safe_layername(layername)
      return layername unless @skpModel.layers[layername]

      layername = if layername =~ /#\d+\Z/
                    layername.succ
                  else
                    layername + " #1"
                  end
      safe_layername(layername)
    end

    def rename_scene_layer(pages)
      page = pages.selected_page
      return unless page
      return if get_memory_attribute(page, "Skalp",
                                     "sectionplaneID") == "" || get_memory_attribute(page, "Skalp",
                                                                                     "sectionplaneID").nil?
      ...
      return unless layer.valid? && layer.name.gsub(/ #\d+\Z/, "") != "#{Skalp.translate('Scene') + ': '}#{page_name}"
    ...
    def sectionplane_in_active_page_match_model_sectionplane?
      get_memory_attribute(@skpModel.pages.selected_page, "Skalp",
    ...
    def activate_sectionplane_by_name(name)
      ...
      @sectionplanes.each_value do |sectionplane|
        sectionplane.sectionplane_name == name && sectionplane.activate && return
      end
    end
    ...
    def new_sectiongroup(skpPage = @skpModel)
      ...
      if skpPage.class == Sketchup::Page
        @sectiongroup.set_attribute("Skalp", "ID",
                                    get_memory_attribute(skpPage, "Skalp", "ID"))
      else
        @sectiongroup.set_attribute(
          "Skalp", "ID", "skalp_live_sectiongroup"
        )
      end
    ...
    def skalp_sections_off
      ...
      @section_result_group.entities.grep(Sketchup::Group).each do |section_group|
        section_group.hidden = true if section_group.get_attribute("Skalp", "ID")
      end
    end
    ...
    def export_dxf_pages(filename, layer_preset, pages = nil)
      ...
      if @hiddenlines.get_page_info_by_index(index) && @hiddenlines.get_page_info_by_index(index)[:parallel] && @hiddenlines.get_page_info_by_index(index)[:sectionplane] # Skalp section and view parallel with section
        ...
        name = if page && page.class == Sketchup::Page
                 page.name
               elsif Sketchup.active_model.pages && Sketchup.active_model.pages.selected_page
                 Sketchup.active_model.pages.selected_page.name
               else
                 ""
               end

        if bounds
          Skalp::DXF_export.new(filename, name, nil, @hiddenlines.forward_lines_result[page], nil,
                                [Skalp.inch_to_modelunits(bounds[:min_x]), Skalp.inch_to_modelunits(bounds[:min_y])], [Skalp.inch_to_modelunits(bounds[:max_x]), Skalp.inch_to_modelunits(bounds[:max_y])], @hiddenlines.get_page_info_by_index(index)[:scale], @linestyle)
        end
      end
    ...
    def manage_sections(skalp_pages, no_skalp_pages)
          ...
          if sectionplane.get_attribute("Skalp", "ID")
            if sectionplane.get_attribute("Skalp", "ID") == sectionplaneID
              if sectionplane.is_a?(Sketchup::Drawingelement) && sectionplane.valid?
                page.set_drawingelement_visibility(sectionplane,
                                                   true)
              end
      ...
      section_result_group.entities.each do |section_group|
        ...
        is_live = section_group.get_attribute("Skalp",
                                              "ID") == "skalp_live_sectiongroup" && Skalp.sectionplane_active == true
    ...
    def prepare_section_result_group
      ...
      @skpModel.entities.grep(Sketchup::Group).each do |group|
        next unless group.get_attribute("Skalp", "section_result_group") == true
      ...
      return if @section_result_group
      ...
      @skpModel.pages.each do |page|
        if get_memory_attribute(page, "Skalp", "sectionplaneID")
          if sectionplane_by_id(get_memory_attribute(page, "Skalp", "sectionplaneID"))
            Skalp.set_ID(page) unless get_memory_attribute(page, "Skalp", "ID")
          else
            delete_memory_attribute(page, "Skalp")
          end
        end
        next unless get_memory_attribute(page, "Skalp", "ID")

        delete_memory_attribute(page, "Skalp") unless get_memory_attribute(page, "Skalp", "sectionplaneID")
      end
    end
    ...
    def correct_sectiongroup(id, layer)
      Sketchup.active_model.entities.grep(Sketchup::Group).each do |group|
        group.layer = layer if group.get_attribute("Skalp", "ID") == id
      end
    end
    ...
    def delete_sectiongroups
      ...
      @skpModel.entities.grep(Sketchup::Group).each do |group|
        ...
        to_delete << group if group.name.include?("Skalp scene section - ") || group.name == "Skalp Active View"
      end
