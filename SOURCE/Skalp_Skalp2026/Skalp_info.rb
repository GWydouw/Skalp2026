module Skalp
  ...
  def self.show_info(active_menu, start_tool = nil, show_guid = false)
    ...
    if Skalp.respond_to?(:online?) && Skalp.online?
    ...
    @info_dialog.add_action_callback("get_guid") do
      if @show_guid
    ...
    @info_dialog.add_action_callback("dialog_ready") do
      ...
      if Sketchup.read_default("RubyWindow", "installation") == true
        show_info_dialog(%i[trial activate release_notes buy])
      else
        show_info_dialog(active_menu)
      end
    end

    @info_dialog.add_action_callback("dialog_wrong_IE") do
    ...
    @info_dialog.add_action_callback("support") do |webdialog, params|
      mail_support
    end

    @info_dialog.add_action_callback("buy") do |webdialog, params|
      buy
    end

    @info_dialog.add_action_callback("open_skalp_store") do |webdialog, params|
      firstname = user_name.split(" ", 2).first
      lastname = user_name.split(" ", 2).last
      url = "https://sites.fastspring.com/Skalp_Skalp2026/instant/skalpforsketchup?contact_fname=#{firstname}&contact_lname=#{lastname}&contact_company=#{user_company}&contact_email=#{user_email}"

      UI.openURL(url)
    end

    @info_dialog.add_action_callback("activate") do |webdialog, params|
      activate_license
    end

    @info_dialog.add_action_callback("trial") do |webdialog, params|
      trial
    end

    @info_dialog.add_action_callback("trial_message") do |webdialog, params|
      if Skalp.respond_to?(:online?) && Skalp.online?
    ...
    @info_dialog.add_action_callback("register_message") do |webdialog, params|
      if params != ""
        ...
        if index == "4"
          ...
          reset_activations(guid) if result == IDYES

        elsif index == "9"
          result = UI.messagebox(register_php_error("9"), MB_YESNO)
          buy_maintenance(guid) if result == IDYES
        else
    ...
    @info_dialog.add_action_callback("update") do |webdialog, params|
      update
    end

    @info_dialog.add_action_callback("skalp_eval") do |webdialog, params|
      eval(Base64.decode64(params))
    end

    @info_dialog.add_action_callback("update_to_new_version") do |webdialog, url|
      ...
      begin
        ...
        Skalp.log.error("Plugin installation interrupt error: #{e}")
        UI.messagebox("#{Skalp.translate('Extension installation was interrupted.')} #{Skalp.translate('Error')}:\n#{e}")
      rescue Exception => e
        Skalp.log.error("Error during rbz installation: #{e}")
    ...
    @info_dialog.add_action_callback("release_notes_menu") do |webdialog, params|
      release_notes
    end
    @info_dialog.add_action_callback("load_release_notes") do |webdialog, params|
      load_release_notes
    end

    @info_dialog.add_action_callback("update_maintenance") do |webdialog, params|
      buy_maintenance(Skalp.guid)
    end

    @info_dialog.add_action_callback("license") do |webdialog, params|
      @info_dialog.set_url("https://#{DOWNLOAD_SERVER}/skalp_info_2_0/skalp_info.html")
      ...
      if File.exist?(Sketchup.find_support_file("Plugins") + "/Skalp_Skalp2026/Skalp.lic") && license_type == "CLASSROOM"
        Skalp.write_classroom_settings
      end
      # @info_dialog.close
    end

    @info_dialog.set_on_close do
      unless @skalp_version_update == :do_not_start_dialog
        ...
        if File.exist?(Sketchup.find_support_file("Plugins") + "/Skalp_Skalp2026/Skalp.lic") && @version_expired == false && license_version > 1
          encoderErrorCheck
          @info_dialog_shown = true
          if start_tool
            case start_tool
  ...
  def self.read_border_size
    ...
    if OS == :MAC
      @w_border = w == true ? 0 : w.to_i
      @h_border = h == true ? 22 : h.to_i
    else
      @w_border = w == true ? 16 : w.to_i
      @h_border = h == true ? 28 : h.to_i
    end
  end
  ...
  def self.show_info_dialog(active_menu)
    ...
    maintenance_renewal if active_menu.first == :maintenance_renewal && !@activated
    ...
    @info_dialog_active = !(@start_tool == :info)
  ...
  def self.send_activation_problem
    ...
    if respond_to? :guid
      old_guid = guid
    elsif Sketchup.read_default("Skalp", "guid")
      old_guid = Sketchup.read_default("Skalp", "guid")
    end
  ...
  def self.create_license_info
    ...
    html.gsub!("expires", "expired") if @version_expired
  ...
  def self.update
    return unless @version_expired || new_version
  ...
  def self.system_info
    ...
    if File.exist?(Sketchup.find_support_file("Plugins") + "/Skalp_Skalp2026/Skalp.lic") && guid
      add_string("License: " + license_type + " (" + guid + ")")
    end
    add_string("")
