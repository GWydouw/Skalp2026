  ...
  class BoxSectionAdjustTool
    ...
    def load_cursors
      ...
      get_cursor = lambda do |name|
        ...
        path = if Sketchup.platform == :platform_osx && File.exist?(pdf)
                 pdf
    ...
    def deactivate(view)
      ...
      Skalp::BoxSection::Engine.update_planes_from_entities(active_id) if active_id
      view.invalidate
    end
    ...
    def onLButtonUp(flags, x, y, view)
      if @state == :check_drag
        ...
        commit_move
        @state = :idle
        # FINAL SYNC: Update all planes when released
        Skalp::BoxSection::SkalpIntegration.update_all if defined?(Skalp::BoxSection::SkalpIntegration)
      end
    ...
    def handle_drag(x, y, view)
      ...
      target.transformation = Geom::Transformation.translation(local_pt) if target

      Sketchup.active_model.commit_operation
      # Refresh data now using optimized cached group
      refresh_data

      # LIVE UPDATE: Recalculate only this plane for speed
      if defined?(Skalp::BoxSection::SkalpIntegration)
        Skalp::BoxSection::SkalpIntegration.update_single_plane(data[:name])
      end
    ...
    def draw(view)
      ...
      Skalp::BoxSection::SkalpDrawHelper.draw_bounds(view, @planes_data,
                                                     { color: Sketchup::Color.new(255, 0, 255), width: 3, stipple: "" })
      ...
      @planes_data.each_with_index do |data, i|
        ...
        Skalp::BoxSection::SkalpDrawHelper.draw_plus(view, pt, data[:normal], data[:name], is_highlighted, model_size,
                                                     plus_color)
      end
    end
