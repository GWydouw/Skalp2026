...
module Skalp
  ...
  class Translator
    ...
    def initialize(filename, options = nil, language = nil)
      @path = nil
      @debug = false
      unless options.nil?
      ...
      @strings = parse(filename, @path, language)
    end
    ...
    #
    # @return [Hash]
    def parse(filename, custom_path = nil, language = Sketchup.get_locale)
      ...
      if custom_path
        ...
        full_file_path = File.join(path, language, filename)
      else
