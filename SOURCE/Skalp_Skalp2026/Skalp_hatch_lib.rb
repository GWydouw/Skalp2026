  ...
  module SkalpHatch
    ...
    class Line2D
      ...
      def update_pts_on_axes
        raise ArgumentError, "ILLEGAL POINTSEQUAL IN LINE2D" if pointsequal?
      ...
      def ==(other)
        p1.x == other.p1.x && p1.y == other.p1.y && p2.x == other.p2.x && p2.y == other.p2.y
      end
    ...
    class Vector2D
      ...
      def define_by_point=(point)
        case point
        when Point2D
          @arrow_point.x = point.x
          @arrow_point.y = point.y
        when Array
          if point.size == 2
            @arrow_point.x = point.first
            @arrow_point.y = point.last
          end
        end
      ...
      def -(other)
        Vector2D.new(Point2D.new(x - other.x, y - other.y))
      end
    ...
    class Transformation2D
      ...
      def *(other) # Transformation2D
        if other.class == Transformation2D
          @transformationmatrix *= other.transformationmatrix
          self
        else
          if defined?(DEBUG) && DEBUG
            puts "transformation not yet implemented for #{other.class}, argument ignored for now"
          end
          self
        end
      end

      def **(other) # exponent.class == integer
        @transformationmatrix = @transformationmatrix **= other
        self
      end
