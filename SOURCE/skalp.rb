  ...
  module Skalp
    def self.remove_wrong_rgloader
      $LOADED_FEATURES.select{ |i| i.include?('rgloader')}.each do |path|
        unless path.include?('skalp')
          $LOADED_FEATURES.delete path
          (defined?(RGLoader)) && Object.send(:remove_const, :RGLoader)
          load 'skalp/Skalp_lic.rb' if File.exist?(Sketchup.find_support_file("Plugins")+"/skalp/Skalp.lic")
        end
      end
    end
    ...
    SKALP_PATH = Sketchup.find_support_file("Plugins")+"/skalp/"
    SKALP_VERSION = '2026.0.0001'
    ...
    if Sketchup.version[0..0].to_i == 9
      SKETCHUP_VERSION = Sketchup.version[0..1].to_i - 70
    else
      SKETCHUP_VERSION = Sketchup.version[0..1].to_i
    end
    ...
    Sketchup::require 'skalp/Skalp_translator'

    options={
        :custom_path => Sketchup.find_support_file("Plugins")+"/skalp/resources/strings/",
        :debug => false
    }
    ...
    def self.translate(string)
      ...
      if translated != nil && translated != '' then
        translated
      else
        string
      end
    ...
    def self.check_IE_version
      ...
      begin
        # IEX10, IEX11
        Win32::Registry::HKEY_LOCAL_MACHINE.open('Software\Microsoft\Internet Explorer', Win32::Registry::KEY_QUERY_VALUE) do |reg|
    ...
    def self.load_extension
      if @ie_check == false
        ...
        @skalp_extension = SketchupExtension.new "Skalp", "skalp/Skalp_loader.rb"
    ...
    if SKETCHUP_VERSION.to_f < @version_required
      UI.messagebox("#{Skalp.translate("You need Sketchup")} 20#{@version_required.to_s} #{Skalp.translate("or higher to run Skalp.")} #{Skalp.translate("Visit sketchup.com to upgrade.")}")
    elsif SKETCHUP_VERSION.to_i > @version_max
      message = "Skalp version #{SKALP_MAJOR_VERSION.to_s} only works with SketchUp 20#{@version_required.to_s}-20#{@version_max.to_s}. Please install the correct SketchUp version or download the correct Skalp version from our website at www.skalp4sketchup.com \n\nIf you have more questions about Skalp requirements, please contact us at support@skalp4sketchup.com"
