  ...
  module Skalp
    def self.remove_wrong_rgloader
      $LOADED_FEATURES.select{ |i| i.include?('rgloader')}.each do |path|
        unless path.include?('Skalp_Skalp2026')
          $LOADED_FEATURES.delete path
          (defined?(RGLoader)) && Object.send(:remove_const, :RGLoader)
          load 'Skalp_Skalp2026/Skalp_lic.rb' if File.exist?(Sketchup.find_support_file("Plugins")+"/Skalp_Skalp2026/Skalp.lic")
        end
      end
    end
    ...
    DEV_LOADER = File.join(File.expand_path(File.dirname(__FILE__)), 'Skalp_Skalp2026', 'dev_loader.rb')
    ...
    if DEV_MODE
       ...
       begin
         ...
         if defined?(Skalp) && Skalp.respond_to?(:load_development_mode)
           ...
           puts "    Skalp.respond_to?(:load_development_mode) = #{Skalp.respond_to?(:load_development_mode) rescue 'ERROR'}"
         end
       rescue => e
